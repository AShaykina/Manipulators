...
class Graph {
    ...
    ArrayList<Point3> dijkstra(short fi1, short fi2, short fi3) {
        ...
        PriorityQueue<Ark3> qu = new PriorityQueue<>((a1, a2) -> {
            ...
            double g = a1.from.distance - a2.from.distance;
        ...
        for (Ark3 t : graph.get(a).list) {
            if (space[t.to.fi1][t.to.fi2][t.to.fi3] != -3) qu.offer(t);
        }
        ...
        while (!qu.isEmpty()) {
            t = qu.poll();
            if (t.to.distance > t.from.distance + 1) {
                t.to.distance = t.from.distance + 1;
                t.to.prev = t.from;
                for (Ark3 k : t.to.list) {
                    if (!k.to.mark && space[k.to.fi1][k.to.fi2][k.to.fi3] != -3)
                        qu.offer(k);
                }
            }
            t.to.mark = true;
